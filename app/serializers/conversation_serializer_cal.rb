class ConversationSerializerCal < ActiveModel::Serializer
  attributes :id, :channel_id, :status, :start_at, :end_at,
             :created_at, :updated_at, :users

  def self.each_json(models)
    models.map {|c| JSON.parse(ConversationSerializerCal.new(c).to_json) }
  end

  def users
    UserInConversationSerializerCal.each_json(object.users)
  end
