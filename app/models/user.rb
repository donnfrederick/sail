...
#  conversation_level       :integer          default(0)
#  country                  :string(191)
#  custom_gender            :string(255) 世界側のみ
#  deleted_at               :datetime
#  desired_condition        :integer
#  encrypted_birthday       :string(191)
#  encrypted_email          :string(191)
#  encrypted_phone_number   :string(191)
#  encrypted_uid            :string(191)
#  evaluation_amazing       :integer          default(0)
#  evaluation_fine          :integer          default(0)
#  evaluation_lovely        :integer          default(0)
#  evaluation_score         :float(24)        default(0.0)
#  evaluation_uncomfortable :integer          default(0)
#  evaluation_very_funny    :integer          default(0)
#  fcm_token                :string(191)
#  highly_reliable          :boolean          default(FALSE)
#  introduce                :text(65535)
#  invalid_as_foreigner     :boolean          default(FALSE)
#  invalid_as_ridiculous    :boolean          default(FALSE)
...
#  picture                  :string(191)
#  preferred_name           :integer
#  provider                 :string(191)
#  rated_conversation_level :integer          default(0)
...
#  organization_device_id   :bigint
#
# Indexes
#
#  index_users_on_auth_token              (auth_token) UNIQUE
#  index_users_on_encrypted_email         (encrypted_email) UNIQUE
#  index_users_on_fcm_token               (fcm_token) UNIQUE
#  index_users_on_organization_device_id  (organization_device_id)
#  index_users_on_type                    (type)
#  index_users_on_web_socket_token        (web_socket_token) UNIQUE
#  users_phone_number_uniq                (encrypted_phone_number) UNIQUE
#
...
class User < ApplicationRecord
  ...
  # for displaying name during conversations
  NAME_PREFERENCES = [
    FULL_NAME      = 1, # フルネーム
    FIRST_NAME     = 2, # 名前
    LAST_NAME      = 3, # 苗字
  ...
  # for ActiveAdmin
  NAME_PREFERENCES_FOR_ADMIN = {
    FULL_NAME    => "フルネーム".freeze,
    FIRST_NAME       => "名前".freeze,
    LAST_NAME      => "苗字".freeze,
  }.freeze
  ...
  SEXES = [
    ...
    SEX_MALE       = 1, # 男性
    SEX_FEMALE     = 2, # 女性
    SEX_PREFER_NOT_TO_SAY = 3, # 言いたくありません
    SEX_APPLICABLE = 9, # その他
  ].freeze
  ...
  SEX_NAMES = {
    ...
    SEX_PREFER_NOT_TO_SAY => "言いたくありません".freeze,
    SEX_APPLICABLE => "その他".freeze,
  }.freeze
  ...
  SEX_NAMES_EN = {
    ...
    SEX_PREFER_NOT_TO_SAY => "Prefer Not to Say".freeze,
    SEX_APPLICABLE => "Other".freeze,
  }.freeze
  ...
  validates :preferred_name, inclusion: { in: NAME_PREFERENCES }, allow_nil: true
  ...
  validates :custom_gender, length: { maximum: 144 }, allow_nil: true
  ...
  def sex_name
    names = ["unknown", "male", "female", "Prefer Not to Say"]
