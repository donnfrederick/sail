...
=begin
        conversations, start_on, end_on = if params[:start_on].exists? && params[:end_on].exists?
          service.list_by_date(params[:start_on], params[:end_on])
        elsif params[:term].exists?
          service.list_by_term(params[:term], params[:page])
        else
          service.list_by_default
        end
=end
        render json: {
          data: ConversationSerializerCal.each_json(conversations),
          include: { users: [] },
          meta: {
            start_on: start_on,
            end_on:   end_on,
...
# Rails.logger.debug "テスト3"
        @reservable_conversations.reject! do |rc|
          if rc.user.organization_sections.select(&:tutoring).present?
            @reservable_conversations.select {|rcx| rcx.start_at.to_i == rc.start_at.to_i && rc.id != rcx.id && rcx.user.organization_sections.select(&:tutoring).empty? }.exists?
          else
            false
          end
        end
# Rails.logger.debug "テスト4"
        render json: ReservableConversationSerializerCal.each_json(@reservable_conversations)
# Rails.logger.debug "テスト5"
      end
      ...
      def report
        ...
        if service.report_from_params(params, block_requested)
          render json: ConversationSerializerCal.new(service.conversation).context_user_report_detailed,
      ...
      def memo
        ...
        if service.memo_from_params(params)
          render json: ConversationSerializerCal.new(service.conversation).context_user_memo,
                 status: :created
        else
          Rails.logger.error service.errors.full_messages
          render_error(service.errors.full_messages, :unprocessable_entity)
