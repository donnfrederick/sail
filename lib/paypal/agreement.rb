module Paypal
  ...
  class Agreement < LooseObject
    attr_reader :id, :name, :description, :status, :start_time, :billing_info, :payer, :plan, :shipping_address, :links,
                :override_merchant_preferences, :agreement_details
    ...
    def self.create!(attrs)
      attrs = attrs.tap do |attrs_|
        if attrs_.fetch(:plan_id).nil? && attrs_[:plan_id].is_a?(Plan)
          attrs_[:plan_id] = { id: attrs_[:plan_id].id }
        end
      ...
      # Paypalサブスクリプションの最新URLに変更
      data = Client.new.api_post("/v1/billing/subscriptions", attrs)
      new(data)
    end

    def self.retrieve!(id)
      # Paypalサブスクリプションの最新URLに変更
      data = Client.new.api_get("/v1/billing/subscriptions/#{id}", {})
      new(data)
    end
    ...
    def cancel!(note)
      data = { note: note }
      Client.new.api_post("/v1/billing/subscriptions/#{id}/cancel", data)
    end
    ...
    def payment_token
      ...
      matches = approval_link.href.match(/I-\w+/)
