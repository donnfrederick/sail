...
export default withConversationId((props: Props) => {
  ...
  const me = conversation ? getMe(conversation, myId) : null

  const partnerName = partner ? getPreferredName(partner, 'partner') : ''
  const myName = me ? getPreferredName(me, 'teacher') : ''
    ...
    <Container>
      ...
      {conversationMode === 'soundOnly' ? (
        <Partner>
          ...
          />
          <PartnerName>{partnerName} さん</PartnerName>
        </Partner>
      ) : null}
      ...
      {partner ? <Nametag name={partnerName} person_type={'partner'} /> : null}
      {me ? <Nametag name={myName} person_type={'self'} /> : null}
      {closeSoon ? (
